"""Structured logging utilities for the model prediction pipeline."""
...
class ModelRunnerLogger:
    """Configure and emit structured logs for the prediction pipeline.

    Each component receives its own logger namespace under ``model_predict.<component>``.
    ...
    ``model_predict`` logger namespace.
    """

    LOG_NAMESPACE = "model_predict"
    _configured = False
    ...
    def configure(cls, level: int | None = None) -> None:
        """Configure stream and CloudWatch handlers for the prediction job.

        Handlers are attached to the ``model_predict`` logger namespace:
    ...
    def _build_cloudwatch_handler(
        ...
        stream_name = os.getenv(
            ...
            f"model-predict-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}",
        )
