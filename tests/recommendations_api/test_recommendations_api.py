...
def test_retriever_ignores_other_users_rows() -> None:
    ...
    class MixedGateway:
        ...
        def get_cold_start_predictions(self, limit: int) -> pd.DataFrame:
            return FakeAwsConnector(mixed_predictions, products).get_cold_start_predictions(
                limit
            )

    retriever = RecommendationsRetriever(MixedGateway())
    frame, cold_start = retriever.retrieve("u_0231", limit=10)
    assert cold_start is False
